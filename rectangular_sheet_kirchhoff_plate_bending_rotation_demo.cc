...
namespace TestSoln
{
  ...
  Vector<double>& t, DenseMatrix<double>& Dn, DenseMatrix<double>& Dt)
 {
  ...
  // The derivatives of the tangent/normal are 0, as normal/tangent does not vary
  // along the edges
}
...
rotate_edge_degrees_of_freedom()
{
 ...
 // We must loop over all elements, as we need to rotate dofs for all
 // elements with nodes on the boundary, not just face elements
 for(unsigned e=0; e<n_element; e++)
  {
   ...
   // Loop nodes  
   unsigned const nnode = el_pt->nnode();
   unsigned nbnode=0 ;
